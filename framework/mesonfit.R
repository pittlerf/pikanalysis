...
#Do the plotting
              plotwitherror(x = (tmin-1):(tmax-1),
                            ...
                            ylab = "C(t)/f(t)",
                            xlab="$t/a$",
                            main=sprintf("$Weight=%4f~Deviation=%4f~t_1=%d~t_2=%d$", normalizedweight[x],sum(newb$cf.tsboot$t0[t1:t2]/coshfit((t1-1):(t2-1),2*L,c(fitresultsrange1[x],fitresultsrange[x])))/length(seq(t1-1,t2-1))-1,t1-1,t2-1)
                      )
              pcol <- col2rgb("red", alpha = TRUE)/255
              pcol[4] <- 0.35
              pcol <- rgb(red = pcol[1], green = pcol[2], blue = pcol[3], alpha = pcol[4])
            ...
            for (t2 in tmax){
              ...
              legend(x="bottomleft",
                ...
                legend=c(sprintf("$a M_\\pi^\\mathrm{eff} = %s $",
                         ...
                         sprintf("$a M_\\pi^\\mathrm{Largest~fit~interval} = %s $",
                              tex.catwitherror(x=finalres,
                                               dx=sd(fit_bs),
                                               digits=3,
                                               with.dollar=FALSE))
...
#Do the plotting
              plotwitherror(x = (tmin-1):(tmax-1),
                            ...
                            ylab = "C(t)/f(t)",
                            xlab="$t/a$",
                            main=sprintf("$Index=%d~Deviation=%4f~t_1=%d~t_2=%d$",which.min(listdev),sum(abs(newb$cf.tsboot$t0[t1:t2]/coshfit((t1-1):(t2-1),2*L,c(fitresultsrange1_false[x],fitresultsrange_false[x]))-listt))/length(seq(t1-1,t2-1)),t1-1,t2-1)
                      )
...
#save(obslistpion,file = "obspion.RData")
