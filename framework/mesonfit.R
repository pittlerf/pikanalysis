...
#Do the plotting
            ...
            if (useCov == FALSE){
              plotwitherror(x = (tmin-1):(tmax-1),
                            ...
                            ylab = "C(t)/f(t)",
                            xlab="$t/a$",
                            main=sprintf("$Deviation=%4f~t_1=%d~t_2=%d$",sum(newb$cf.tsboot$t0[t1:t2]/coshfit((t1-1):(t2-1),2*L,c(fitresultsrange1[x],fitresultsrange[x])))/length(seq(t1-1,t2-1))-1,t1-1,t2-1)
                      )
            }
            pcol <- col2rgb("red", alpha = TRUE)/255
            pcol[4] <- 0.35
